...
app.post('/appointments', (req, res) => {
  const { customer_id, customer_name, phone, email, service_id, technician_id, date, time_slot } = req.body;
  db.run(`INSERT INTO appointments (customer_id, customer_name, phone, email, service_id, technician_id, date, time_slot) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [customer_id, customer_name, phone, email, service_id, technician_id, date, time_slot], function(err) {
    if (err) {
...
app.get('/appointments', (req, res) => {
  ...
  if (!customerId) {
    res.status(400).send('Missing customer_id');
    return;
  }
  db.all('SELECT a.*, s.name as service_name, t.name as technician_name FROM appointments a JOIN services s ON a.service_id = s.id JOIN technicians t ON a.technician_id = t.id WHERE a.customer_id = ?', [customerId], (err, rows) => {
    if (err) {
